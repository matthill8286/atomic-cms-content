import { FC } from 'react';
import { ProfileSectionProps, ProfileSectionsProps } from '../ProfileTileColumns/ProfileTileColumns.interface';
export declare const SpeakerHeader: ({ title, name, marginTop }: {
    title: any;
    name: any;
    marginTop: any;
}) => JSX.Element;
export declare const ProfileTileRow: FC<ProfileSectionProps>;
export declare const BrandedHeadingWrapper: any;
export declare const ProfileTileRows: FC<ProfileSectionsProps>;
//# sourceMappingURL=index.d.ts.map